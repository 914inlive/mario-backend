...
function filterAsyncRouter (routes) {
  const USER_TMP_ROUTER = db.get('USER_TMP_ROUTER', '');
  let showKeys = ['404', 404];
  for (let i = 0; i < USER_TMP_ROUTER.length; i++) {
    if (USER_TMP_ROUTER[i].showed == 1)
      showKeys.push(USER_TMP_ROUTER[i].name);

    for (let j = 0; j < USER_TMP_ROUTER[i].children.length; j++) {
      if (USER_TMP_ROUTER[i].children[j].showed == 1)
        showKeys.push(USER_TMP_ROUTER[i].children[j].name);
    }
  }

  for (let i = 0; i < routes.length; i++) {
    routes[i].hidden = !(showKeys.includes(routes[i].name));
  }
