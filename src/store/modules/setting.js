...
export default {
  ...
  state: {
    ...
    sidebar: {
      opened: db.get('SIDEBAR_STATUS', false),
  ...
  mutations: {
    ...
    toggleChatBar: state => {
      state.chatBar.opened = !state.chatBar.opened
      state.chatBar.withoutAnimation = false
      if (state.chatBar.opened) {
        db.save('CHATBAR_STATUS', 1)
      } else {
        db.save('CHATBAR_STATUS', 0)
      }
    },
    ...
    closeChatBar: (state, withoutAnimation) => {
      db.save('CHATBAR_STATUS', 0)
      state.chatBar.opened = false
      state.chatBar.withoutAnimation = withoutAnimation
    },
